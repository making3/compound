    ...
    process.nextTick(function() {
        if (!compound.initialized) {
            ...
            if (isServerSide && compound.app) {
                compound.configure(compound.root, function () {
...
/**
 * Run app configutators in `config/environment`.
 * @param {Compound} compound - compound app descriptor.
 * @param {function} callback - callback when finished.
 */
Compound.prototype.configure = function configureApp(root, callback) {
    var compound = this;
    ...
    if (!requireIfExistsAsync(mainEnv + '.js', compound, compound.configureEnvironment.bind(this, root, callback))) {
        requireIfExistsAsync(mainEnv + '.coffee', compound, compound.configureEnvironment.bind(this, root, callback));
    }
};


/**
 * Run app configutators in `config/environments/env`.
 * @param {Compound} compound - compound app descriptor.
 * @param {function} callback - callback when finished.
 */
Compound.prototype.configureEnvironment = function configureEnvironment(root, callback) {
    var compound = this;
    var app = compound.app;
    var supportEnv = app.root + '/config/environments/' + app.settings.env;
    if (!requireIfExistsAsync(supportEnv + '.js', compound, callback)) {
        if (!requireIfExistsAsync(supportEnv + '.coffee', compound, callback)) {
            callback();
        }
    }
};
...
/**
 * Require `module` if it exists async
 *
 * @param {String} module - path to file.
 * @param {Compound} compound - express app.
 * @param {function} callback - callback when finished.
 * @return {Boolean} success - returns true when required file exists.
 */
function requireIfExistsAsync(module, compound, callback) {
    if (fs.existsSync(module)) {
        requireFun(module)(compound, callback);
        return true;
    } else {
        return false;
    }
